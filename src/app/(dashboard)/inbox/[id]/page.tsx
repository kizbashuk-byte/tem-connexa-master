...
export default async function ConversationPage({
  ...
  const { data: conv, error: convError } = await supabase
    ...
    .select(`
      ...
      status,
      last_message_at,
      customers ( id, name, phone, whatsapp_id, email, messenger_psid, instagram_id ),
  ...
  const attendeeName = customer?.name || customer?.whatsapp_id || customer?.instagram_id || customer?.messenger_psid || customer?.phone || "Unknown Attendee";
            ...
            <div>
              <h2 className="font-bold text-gray-900 leading-tight flex items-center gap-2">
                ...
                {customer?.instagram_id && (
                  <span title="Instagram Direct" className="inline-flex items-center justify-center w-4 h-4 rounded text-pink-600">
